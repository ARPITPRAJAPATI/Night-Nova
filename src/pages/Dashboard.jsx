      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex justify-between items-end gap-8 mb-16 border-b border-white/20 pb-8">
          ...
          <div className="border border-white/20 px-6 py-4 flex gap-8 bg-black/50">
             ...
             <div><div className="text-[9px] font-mono text-white/30 uppercase tracking-widest mb-1">Visits</div><div className="text-sm font-bold text-white">{Array.isArray(bookings) ? bookings.length : 0}</div></div>
          </div>
        </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {bookings.map((booking, i) => (
              <motion.div key={booking._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-black/80 overflow-hidden flex flex-col border border-white/20 hover:border-white transition-colors">
                ...
                <div className="p-6">
                  <h3 className="text-3xl font-bold text-white mb-1 uppercase tracking-tighter">{booking.venueId?.name || 'Unknown Venue'}</h3>
                  <div className="flex items-center gap-2 text-white/40 text-[10px] uppercase font-mono tracking-widest mb-6"><MapPin size={12} /> {booking.venueId?.city || 'Unknown City'}</div>
